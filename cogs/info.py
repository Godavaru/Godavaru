...
Host               :  heroku```""".format(commands, cogs, self.bot.version, version, pversion, ping, self.get_bot_uptime(), server_count, member_count, channel_count))
    ...
    async def avatar(self, ctx):
        ...
        **Permission:** User"""
        ...
        avi = user.avatar_url
        avi = avi.replace("gif?size=1024", "gif")
        avi = avi.replace("webp?size=1024", "png?size=1024")
        if avi == "":
            avi = user.default_avatar_url
        embed = discord.Embed(title=user.display_name+"'s avatar!",description="Click [here]("+avi+")!",color=ctx.author.color).set_image(url=avi).set_footer(icon_url=avi, text="Requested by "+ctx.author.display_name)
            ...
            try:
                if user == getInfo:                                                             
                    if getInfo.id not in [member.id for member in ctx.message.guild.members]:   # the `user == getInfo` is a check to see
                        ...
                        sorted_roles = [r.name for r in sorted(user.roles, key=lambda x: -x.position) if not r.is_default()]
