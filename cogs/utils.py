...
class Utils():
    ...
    async def dog(self, ctx):
        ...
        **Permission:** User"""
        ...
        content = [":dog: Don't be sad! This doggy wants to play with you!",
                   "You seem lonely, {0.mention}. Here, have a dog. They're not as nice as cats, but enjoy!".format(ctx.author),
    ...
    async def jumbo(self, ctx):
        ...
        **Permission:** User"""
        ...
        emote_id = None
        try:
            if extract_emote_id(emote) is not None and extract_emote_name(emote) is not None:
                emote_id = extract_emote_id(emote)
                emote_name = extract_emote_name(emote)
        except:
            pass
        if emote_id is None:
            await ctx.send(":x: That is not a custom emote.")
        ...
        r = o.open(emote_url)
        data = r.read()
        with open("./images/emotes/{}.png".format(emote_name), "wb") as avatar:
            avatar.write(data)
            avatar.close()
        await ctx.send(file=discord.File("./images/emotes/{}.png".format(emote_name)))
